...
class EnemyAI:
    ...
    def spawn_enemies(self):
        ...
        spawn_points_touched_list = arcade.check_for_collision_with_list(self.game_view.player,
                                                                         self.game_view.scene[LAYER_NAME_SPAWN_TRIGGER])
        if spawn_points_touched_list != []:
            ...
            enemy = EnemyCharacter(enemy_name, enemy_hp, enemy_damage, self.game_view.player, self.game_view.scene[LAYER_NAME_WALLS])
            ...
            self.game_view.scene[LAYER_NAME_ENEMIES].append(enemy)
    ...
    def get_damage_from_enemy(self):
        ...
        enemies_hit_list = arcade.check_for_collision_with_list(self.game_view.player,
                                                                self.game_view.scene[LAYER_NAME_ENEMIES])
        ...
        # If player's health reaches 0, she respawns at starting coordinates with full health (for now)
            ...
            self.game_view.player.center_x = PLAYER_START_X
            self.game_view.player.center_y = PLAYER_START_Y
